...
class DeploymentBucketPlugin {
  ...
  async applyDeploymentBucket() {
    try {
      ...
      if (await this.hasChangedBucketTags(this.deploymentBucket.name, this.config.tags)) {
      ...
      if (await this.hasChangedPublicAccessBlock(this.deploymentBucket.name, this.config.blockPublicAccess)) {
